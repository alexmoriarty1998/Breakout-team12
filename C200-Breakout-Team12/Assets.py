...
class Assets:
	...
	# the list of one picture is converted to colored & fading to transparent image list in AssetLoaderHelper
	A_BRICK_FRAG_1 = Animation([li("animations/brickFrags/frag1")], 7, 0)
	A_BRICK_FRAG_2 = Animation([li("animations/brickFrags/frag2")], 7, 0)
	A_BRICK_FRAG_3 = Animation([li("animations/brickFrags/frag3")], 7, 0)
	...
	# won game confetti effect (colorized and alpha'd in AssetLoaderHelper like the brick fragments)
	pass
...
class AssetLoaderHelper:
	# Initializes values in Assets using setattr and loops.
	...
	# load numbers and letters into imgFont
	...
	# The three fragTypes have just one image in their list of frames.
	# Take this white template image, colorize it to the brick color, and
	# make multiple copies that lighten up over time. Do this for each brick
	# type (for i in "1234AC", same format as used in level files), so each
	# brick has an appropriately colored & fading out fragment of each type
	# (1, 2, 3). Save these as A_BRICK_FRAG_1A, where 1 is the frag type (1-3),
	# and A is the brick code (so A_BRICK_FRAG_32 for frag type 3 and brick
	# type 2 = red 2 HP brick).
	for fragType in range(1, Assets.NUM_BRICK_FRAG_TYPES + 1):
		for i in "1234AC":
			...
			for a in [255, 204, 153, 102, 51]: # alpha values, so animation fades out
				innerColor = color + (a,)
				oldImage: Surface = oldAnim.images[0].copy()
				oldImage.fill(innerColor, None, pygame.BLEND_RGBA_MULT)
